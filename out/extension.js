...
let pendingInitialQuestion; // temp storage for the question typed before creating session
...
function safeAnswerFilename(doc, fallbackBase) {
...
async function ensureSocket() {
    ...
    ws.on('message', async (raw) => {
        ...
        switch (msg.type) {
            ...
            case 'created': {
                const sid = asString(msg.sessionId);
                if (!sid)
                    return;
                sessionId = sid;
        ...
        try {
            ...
            // -------------------- Join (Student) --------------------
                ...
                if (sessionId && myRole === 'student') {
                    isJoining = false;
                    void vscode.window.showWarningMessage(`Already in session ${sessionId}. Leave it first before joining another.`);
                    return;
                }
                myRole = 'student';
